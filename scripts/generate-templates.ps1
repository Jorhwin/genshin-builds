...
<BaseLayout title={title} description={desc}>
  <p><span class="tag tag-S">Build Analysis</span></p>
  <h1>{a.title}</h1>
...
`;
fs.writeFileSync(analysisDir + "/[slug].astro", analysisTemplate, "utf-8");
...
<BaseLayout title={title} description={desc}>
  <p><span class="tag tag-S">Game Mechanic Guide</span></p>
  <h1>{g.title}</h1>
...
`;
fs.writeFileSync(guideDir + "/[slug].astro", guideTemplate, "utf-8");
